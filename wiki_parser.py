...
def wiki_scraper():
    try:
        with open("datasets/Kapodistria_scheme/Wiki_Object", 'rb') as wo:
            page = pickle.load(wo)
        ...
        if page.status_code == 200:
            with open("datasets/Kapodistria_scheme/Wiki_Object", 'wb') as wo:
                pickle.dump(page, wo)
    ...
    rc.to_csv("datasets/Kapodistria_scheme/Regions_Counties.csv", sep='\t', columns=rc.columns, index=False)

    cm = pd.DataFrame(counties_municipalities)
    cm.to_csv("datasets/Kapodistria_scheme/Counties_Municipalities.csv", sep='\t', columns=cm.columns, index=False)
